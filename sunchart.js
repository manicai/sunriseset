...
function plot_sunchart(data_file, parent_selector, mark_dates) {  
    ...
    var midday = d3.svg.line()
        .x(function (d) { return x(d.date); })
        .y(function (d) { 
            return y(new Date((d.sunrise.getTime() 
                                 + d.sunset.getTime()) / 2)); 
        });

    if (typeof mark_dates === 'undefined') {
        mark_dates = [];
    }

    d3.csv(data_file, read_row, function (error, data) {
        ...
        y.domain([time_of_day(4, 0), time_of_day(22, 0)]);
        ...
        for (var i = 0; i < mark_dates.length; i += 1)
        {
            var index = find_index(mark_dates[i].date, data),
                mark = data[index];
            ...
            svg.append('line')
               .attr('class', 'solstice')
               .attr('x1', x(mark.date))
               .attr('y1', y(mark.sunrise))
               .attr('x2', x(mark.date))
               .attr('y2', y(mark.sunset));

            // I need to understand SVG transforms better ...
            svg.append('text')
               .attr('transform', ('translate(' + (x(mark.date) + 5) + ',' 
                                                + (y(mark.sunset) + 5) + ') ' 
                                   + ' rotate(-90)'))
               .attr('dy', '.71em')
               .style('text-anchor', 'end')
               .text(mark_dates[i].text);
        }
...
plot_sunchart('suntimes.txt', 'div#winter-chart',
              [{date: new Date(2015, 11, 22), text: 'Winter Solstice'},
               {date: new Date(2016,  2, 20), text: 'Spring Equinox'}]);
plot_sunchart('year_suntimes.txt', 'div#year-chart', 
              [{date: new Date(2016,  2, 20), text: 'Spring Equinox'},
               {date: new Date(2016,  5, 21), text: 'Summer Solstice'},
               {date: new Date(2016,  8, 22), text: 'Autumn Equinox'},
               {date: new Date(2016, 11, 21), text: 'Winter Solstice'}]);